...
Development defaults: 8 global active runs, 3 per credential, 30 minute awaiting TTL, 10 minute replay TTL, 60 minute run deadline, 40 seconds to the first SDK event, 1.5 s tool-batch settle cap (`TOOL_BATCH_SETTLE_MS`) with a 200 ms idle close (`TOOL_BATCH_IDLE_MS`), 15 s SSE keep-alive (`SSE_HEARTBEAT_MS`), and `HOST_SYSTEM_PROMPT_MODE=inline` (`replace` is opt-in, with inline fallback for gated accounts).
