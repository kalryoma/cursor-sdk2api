...
A tool batch closes the way OpenCodex's Cursor adapter closes its client-tool turn, from the SDK's own signals rather than a fixed debounce. `onDelta` forwards `partial-tool-call` / `tool-call-started` for custom (`mcp`) calls as `tool-call-announced`, and `step-started` / `step-completed` for the model call; `SDKCustomToolContext.toolCallId` is the same id as the announce `callId`. The pump keeps the set of announced calls whose `execute` has not fired. A batch is published when that set is empty and the SDK has been idle for `TOOL_BATCH_IDLE_MS` (default 200; every delta or callback re-arms it, an announce disarms it), or at once on `step-completed`. `TOOL_BATCH_SETTLE_MS` (default 1500) after the latest `execute` remains the cap and fires whatever the signals said, so an announce whose execute never arrives or does not match costs nothing worse than the old behaviour. Calls in one generation still arrive separately (~0.1–0.7 s), with `token-delta` between them. `turn-ended` waits for `execute` promises (this gateway holds them until the client posts `tool_result`). SDK output that arrives after the batch was published is carried, in order, into the next segment: a tool call opens it as its own batch, text or thinking heads its journal. Continuation and lineage use only the published batch. Logs: `batch_close` (`settle_timer` / `idle` / `step_completed` / `carried`), `batch_close_wait_ms`, `announce_lead_ms`, `tool_spread_ms`, `tool_result_gap_ms`; segment timings restart on every continuation. Live main→PR numbers (idle ~200 ms close, text stop on `turn-ended`): [pr-performance-diff.md](evidence/pr-performance-diff.md). Whether announces precede executes on a live model shows up as `announce_lead_ms` in `npm run live:timing`.

The final boundary is published on the SDK `turn-ended` delta, with that turn's usage, when the journal already holds streamed text; `run.stream()` EOF and `run.wait()` then only close the loop and cannot replace the published final. A turn whose text arrives only in the run result still completes through the EOF path. Logs: `turn_ended_ms`, `run_settled_ms`, `publish_lag_ms` (model's last output to the published stop).
...
`@cursor/sdk` 1.0.31 keeps the 1.0.30 public delta schema, `coreToSdkUpdate`, `Agent.create` / `resume` / `send`, and `Run.stream` / `wait` / `cancel`. New: `AgentOptions.systemPrompt` (account-gated; used by `HOST_SYSTEM_PROMPT_MODE=replace`) and unused `SDKCustomTool.annotations`. Sand `x-cursor-client-type` sites are rehashed in `sand-patch-contract.ts`; do not reuse 1.0.30 hashes.
