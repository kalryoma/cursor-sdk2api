...
A tool batch closes `TOOL_BATCH_SETTLE_MS` after the latest `execute`, or earlier on SDK delta silence when `TOOL_BATCH_IDLE_MS` is set (settle stays the cap). `@cursor/sdk` 1.0.30 and 1.0.31 have no generation-end event before local tools run: `turn-ended` waits for `execute` promises (this gateway holds them until the client posts `tool_result`), and `tool-call-started` is dispatch-only. Calls in one generation arrive separately (~0.1–0.7 s). Between calls the model still emits `token-delta`; after the last call the stream goes silent. `TOOL_BATCH_IDLE_MS=300` cuts stop wait to ~300 ms and can split a slow Sonnet parallel batch; default stays off. SDK output that arrives after the batch was published is carried, in order, into the next segment: a tool call opens it as its own batch, text or thinking heads its journal. Continuation and lineage use only the published batch. Logs: `batch_close` (`settle_timer` / `idle` / `carried`), `batch_close_wait_ms`, `tool_spread_ms`, `tool_result_gap_ms`; segment timings restart on every continuation. Live before/current numbers: [pr-performance-diff.md](evidence/pr-performance-diff.md).
