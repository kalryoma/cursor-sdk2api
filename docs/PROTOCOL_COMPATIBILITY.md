...
| client system prompt | `HOST_SYSTEM_PROMPT_MODE` | `replace` (default): Messages `system`, Chat `system`/`developer`, and Responses `instructions`/`developer` become the SDK `systemPrompt` on `Agent.create` and every `Agent.resume`, replacing Cursor's built-in harness prompt; the harness tool context is restated when tools exist and the inline `System:` block is dropped. Requests without a system prompt keep Cursor's default. An account without `systemPrompt` access fails the first `send` naming `--system-prompt`; the gateway retries that request inline before any output and remembers the credential. `inline`: the prompt travels inside the first user send, as before. Logs carry `system_prompt_mode`, never the text. |
