...
| `/v1/responses` | yes | Protocol adapter over the same Messages run engine. Contract-tested: non-stream text; Responses SSE lifecycle (`response.created` → deltas → `response.completed`); reasoning summary events; base64 `input_image`; top-level and `additional_tools` client function declarations; custom/freeform tools with native `custom_tool_call` events; top-level and `additional_tools` namespace wrappers (empty groups are ignored; children qualify to SDK-safe names and restore Responses identity); top-level-authoritative Responses Lite dedupe; same-turn parallel calls; full-history `function_call_output` / `custom_tool_call_output` continuation by the latest trailing `call_id` batch; duplicate-same replay; deferred/final cache-aware usage; `reasoning`/`reasoning_effort` / `cursor_model_params`; unknown/mixed/missing `call_id` fail closed; OpenAI REST errors before stream start and a Responses `error` event after stream start. `text.format.type=json_schema` is forwarded as a prompt-level output contract because the official Cursor SDK has no structured-output enforcement surface; callers must validate strict schemas. Tool outputs accept a string or text-content array; image/file tool-output parts are `422` until native SDK mapping is implemented. `previous_response_id`, `store=true`, background, conversation, remote image URLs, and provider-hosted tools fail closed. The known optional `reasoning.encrypted_content` include is accepted but omitted; unknown include expansions fail closed. Completed Responses `usage` always includes cache and reasoning detail objects. Grok 4.6 Responses named-tool, same-turn parallel, client-workspace file tool, and kill/restart recovery have live acceptance evidence. |
...
| `output_config.effort` | supported | Claude Code effort maps to the official SDK `effort` model parameter. |
| client system prompt | `HOST_SYSTEM_PROMPT_MODE` | `inline` (default): the prompt stays in the first user send. `replace`: Messages `system`, Chat `system`/`developer`, and Responses `instructions`/`developer` become the SDK `systemPrompt` on create and resume, bound to the session by digest; a changed prompt on a live session resumes the agent with it; a mid-turn restart resume re-applies the stored prompt in its stored mode, and without it is `409 cursor_session_conflict`. A gated account retries inline and is remembered. Logs carry `system_prompt_mode`, never the text. |
