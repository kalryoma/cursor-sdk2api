...
- Add `npm run live:cli-timing` and `npm run live:harness-vs-cli`. The latter is the same-work text PONG: Claude Code / Codex / Grok Build protocols through this gateway versus official Cursor CLI, with fast when both catalogs expose it. Charts are in `docs/evidence/pr-performance-diff.md`.
