...
- Tool items stream when the SDK requests the tool; stream, non-stream body, replay, and `response.completed.output` share one ordered journal, including events that fire before `send()` resolves. Only the stop reason waits for the batch to close. Live `6951f1b` vs this tree (same client, 2026-09-07 redo): `docs/evidence/2026-09-07-pr-performance-diff.md`. Earlier same-suite pass: `docs/evidence/2026-09-06-pr-performance-diff.md`.
